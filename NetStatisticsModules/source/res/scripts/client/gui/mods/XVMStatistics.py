...
XVM_SERVER  = 'https://stat.modxvm.com:443/{API}/{REQ}'
...
# "status":"active"}
XVM_GETTOKEN = XVM_SERVER.format(API=API_VERSION, REQ='getToken/{TOKEN}/{ID}')
...
XVM_GETVERSION = XVM_SERVER.format(API=API_VERSION, REQ='getVersionWithLimit/{TOKEN}/{ID}/50')
...
XVM_STATSBYID = XVM_SERVER.format(API=API_VERSION, REQ='getStatsByID/{TOKEN}/{ID}')
...
XVM_STATSBYNICK = XVM_SERVER.format(API=API_VERSION, REQ='getStatsByNick/{TOKEN}/{REGION}/{NICK}')
...
XVM_STATS       = XVM_SERVER.format(API=API_VERSION, REQ='getStats/{TOKEN}/{DICT}')
XVM_STATSREPLAY = XVM_SERVER.format(API=API_VERSION, REQ='getStatsReplay/{TOKEN}/{DICT}')
...
XVM_ONLINE = XVM_SERVER.format(API=API_VERSION, REQ='getOnlineUsersCount/{TOKEN}')
            ...
            if path.exists(accountDBID):
                try:
                    with open(accountDBID, 'rb') as f:
                        accountDBID = cPickle.loads(f.read())
                except:
                    accountDBID = None
            else:
                accountDBID = None
        ...
        if path.exists(tokensBase):
            try:
                with open(tokensBase, 'rb') as f:
                    tokensBase = cPickle.loads(f.read())
            except:
                tokensBase = {}
        else:
            tokensBase = {}
    ...
    def __getLocalToken(self):
        ...
        if path.exists(localToken):
            try:
                ...
                localToken = localToken.get('token','') if isinstance(localToken, dict) else ''
            except:
                localToken = ''
        else:
            localToken = ''
